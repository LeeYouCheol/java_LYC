...
function getCommentList(cri){
	//cri는 객체
	//크리가 언디파인드이거나 크리가 널 이거나 객체가 아닌경우
...
function commentListSuccess(data){
	...
	//반복문을 이용하여 댓글 구성
	for(co of list){
		str += '<div class="media border comment-body p-3">'
		str += 		'<div class="media-body">'
		...
		str +=			'<button data-orinum="'+co.co_ori_num+'" data-depth="'+co.co_depth+'" data-order="'+co.co_order+'" class="btn btn-outline-warning btn-co-reply mb-1" style="display: block">답글</button>'
		...
		str +=			'<button data-target="'+co.co_num+'" class="btn btn-outline-danger mt-1 btn-co-delete" style="display: block">삭제</button>'
		}
		str +=		'</div>'
		str += '</div>'
	}
	...
	$('.btn-co-update').click(function(){
		$('.btn-co-cancel').click();
		$('.btn-reply-cancel').click();
		contentEl = $(this).parent().siblings('.media-body').children('p').hide();
		contentEl.hide();
		...
		str +=		'<button class="btn btn-outline-danger mt-1 btn-co-cancel" style="display: block">취소</button>'
		str += '</div>'
