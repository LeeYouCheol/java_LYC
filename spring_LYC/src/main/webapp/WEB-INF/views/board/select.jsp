	...
	<div class="container">
		<c:if test="${board != null && 'N'.charAt(0) == board.bd_del}">
			...
			<div class="form-group mt-3">
				...
				<button class="btn btn-outline-primary col-12 btn-comment-insert">댓글등록</button>
			</div>
	...
	$(function(){
		...
		$(document).on('click', '.btn-comment-update-complete', function(){
			...
			$(document).on('click', '.btn-comment-reply', function(){
				let id = '${user.me_id}';
				if(id == ''){
					if(confirm('댓글등록은 로그인한 회원만 가능합니다. 로그인 하겠습니까?')){
						location.href = '<%=request.getContextPath()%>/login'
						return;
					}
				}
				//답글을 누른 댓글에만 답글을 입력하는 창이 나오게 하기위해 모든 답글취소 버튼을 클릭
				$('.btn-cancel-reply').click();
				$('.btn-comment-update-cancel').click();
				...
				$(this).after(str);
				$(this).hide(); //답변버튼 감춤
				$(this).siblings('.btn-comment-update').hide(); //수정버튼 보임
				$(this).siblings('.btn-comment-delete').hide(); //삭제버튼 보임
			})
			//답글 등록버튼 클릭
			...
			$.ajax({
				...
				success : function(data){
					...
					'<div class="item-comment">' +
						...
						'<div class="co_content" style="font-size : 20px">'
