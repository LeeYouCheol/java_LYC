...
<div class="container">
	<h2>제품 리스트</h2>
	<a href="<c:url value="/admin/product/insert"></c:url>" class="btn btn-outline-info mb-3">제품등록</a>
	<table class="table table-hover">
    	...
    	<c:forEach items="${list}" var="pro">
			<tr>
		        ...
		        <td>${pro.pr_code}</td>
		        <td>
		        	<a href="<c:url value="/product/select?pr_code=${pro_pr_code}"></c:url>">${pro.pr_title}</a>
		        </td>
		        <td>${pro.pr_price}</td>
				<td>
			        <a class="btn btn-outline-primary" href="<c:url value="/product/update?pr_code=${pro_pr_code}"></c:url>">수정</a>
			        <a class="btn btn-outline-danger" href="<c:url value="/product/update?pr_code=${pro_pr_code}"></c:url>">삭제</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
	...
	<ul class="pagination justify-content-center">
    	...
    	<c:forEach begin="${pm.startPage}" end="${pm.endPage}" var="i">
    		<li class="page-item <c:if test="${!pm.prev }">active</c:if>">
		...
		<li class="page-item <c:if test="${!pm.prev }">disabled</c:if>">
			<a class="page-link" href="<c:url value="/admin/product/list?page=${finalPage}&search=${pm.cri.search}&pr_ca_name=${pm.cri.pr_ca_name}"></c:url>">마지막</a>
		</li>
	</ul>
	<form>
  		<div class="input-group mb-3">
			...
			<input type="text" class="form-control" placeholder="제품 제목 또는 제품코드로 검색하세요" name="search" value="${pm.cri.search}">
