...
public class Ex7_Exceptiond_TryCatchFinally {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 * - 예외가 발생하면 밑에 코드가 있던 없던 상관없이 예외를 처리할수 있는 catch로 이동해서
			 *   (예외를 처리할 수 있는 catch가 없는 경우 처리할 수 없어서 프로그램 중단) 예외를 처리함
			 * - catch는 여러개 올수 있고, Exception을 처리한다면 Exception은 무조건 마지막 catch에 배치해야함
		...
		// 위에 Exception이 있어서 이부분까지 올일이 없다. 
